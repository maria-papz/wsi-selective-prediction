"""
ABMIL training loop for IDH classification on EBRAINS.

Usage:
    python scripts/training/train_abmil.py --encoder uni2 --fold 0
    python scripts/training/train_abmil.py --encoder conch --fold 1 --epochs 50
"""
...
def main():
    ...
    parser.add_argument("--wandb_project",type=str,   default="idh-abmil")
    ...
    run_name = args.run_name or f"{args.encoder}_fold{args.fold}_seed{args.seed}"
    wandb.init(
        ...
        name=run_name,
        config=vars(args),
        tags=[args.encoder, f"fold{args.fold}"],
    )
    ...
    ckpt_dir = BASE / f"outputs/abmil/{args.encoder}/fold{args.fold}"
    ...
    artifact = wandb.Artifact(f"abmil-{args.encoder}-fold{args.fold}", type="model")
