...
subset) ABMIL models on IPD Brain - a second, independent external cohort
(different site/scanner/diagnostic era than TCGA), held out entirely from
training.

Codel labels here are a Subtype-based proxy (OLIGODENDROGLIOMA vs
ASTROCYTOMA, IDH-mutant only), built by build_1p19q_labels.py's
build_ipd_brain(), not a direct genomic assay like TCGA's -- report
alongside the TCGA result, not as a replacement for it.

Usage:
    python scripts/training/eval_codel_ipd_brain.py --encoder uni2
"""
...
def load_ipd_brain_codel(encoder: str) -> pd.DataFrame:
    slide_table = pd.read_csv(BASE / f"data/processed/slide_table_ipd_brain_{encoder}.csv")
    clinical    = pd.read_csv(BASE / "data/raw/ipd_brain/clinical/ipd_brain_1p19q_labels.csv")
    slide_table = slide_table.merge(
        clinical[["case_id", "codel_binary"]],
...
def main():
    ...
    parser.add_argument("--feature_root", type=str, default="data/processed/features/ipd_brain",
    ...
    slide_table = load_ipd_brain_codel(args.encoder)
    print(f"IPD Brain codeletion-labelled slides: {len(slide_table)}")
    ...
    for fold in range(args.n_folds):
        ...
        print(f"[fold {fold}] IPD Brain external AUROC = {auroc:.4f}  95% CI [{lo:.4f}, {hi:.4f}]  "
              f"(train val_auroc was {ckpt['val_auroc']:.4f})")

        fold_results.append({
            "fold": fold,
            "ipd_brain_auroc": auroc,
            "ipd_brain_auroc_ci95": [lo, hi],
    ...
    fold_aurocs = [r["ipd_brain_auroc"] for r in fold_results]
    print("\n=== Summary ===")
    print(f"Per-fold IPD Brain AUROC: mean={np.mean(fold_aurocs):.4f} std={np.std(fold_aurocs):.4f} "
          f"({[round(a,4) for a in fold_aurocs]})")
    print(f"Ensemble (mean-prob) IPD Brain AUROC = {ensemble_auroc:.4f}  95% CI [{lo:.4f}, {hi:.4f}]")

    out_dir = BASE / f"outputs/abmil_codel/{args.encoder}"
    out_dir.mkdir(parents=True, exist_ok=True)
    with open(out_dir / "ipd_brain_external_eval.json", "w") as f:
        json.dump({
            "n_ipd_brain": len(slide_table),
            "ipd_brain_class_balance": slide_table["codel_status"].value_counts().to_dict(),
    ...
    print(f"\nWrote {out_dir / 'ipd_brain_external_eval.json'}")
