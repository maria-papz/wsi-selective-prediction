"""
MC-dropout uncertainty for a locked ABMIL checkpoint.

Loads one encoder/fold's already-trained best.ckpt (see train_abmil.py) and runs
inference with dropout left ACTIVE (model.train(), no backward pass) instead of the
usual single deterministic model.eval() pass. Repeating this `n_passes` times per
slide gives a distribution over prob_mutant per patient; averaging that distribution
is the MC-dropout predictive probability, and its spread is the MC-dropout epistemic
signal -- a second, architecturally-independent uncertainty estimate to compare
against the encoder/fold ensemble disagreement already used elsewhere in the bench.

ABMIL has no batchnorm (only nn.Dropout in `proj` and `classifier`), so model.train()
only randomises dropout masks -- it does not touch any running statistics the way it
would if the model had batchnorm layers.

The decomposition mirrors load_abmil_ensemble() in the bench notebook:
...
Usage:
    python scripts/training/mc_dropout_infer.py --encoder uni2 --fold 0 --dataset tcga
    python scripts/training/mc_dropout_infer.py --encoder conch --fold 2 --dataset tcga --n_passes 100
"""
...
def mc_dropout_pass(model: ABMIL, feats: torch.Tensor, n_passes: int) -> np.ndarray:
    """T stochastic forward passes over one already-loaded feature bag.
    Returns prob_mutant per pass, shape [T]."""
...
def deterministic_pass(model: ABMIL, feats: torch.Tensor) -> float:
    """Single eval-mode (dropout off) pass, kept alongside the MC estimate as a
    sanity check that dropout-averaging isn't drifting the prediction."""
...
def main():
    ...
    parser.add_argument("--wandb_project", type=str, default="idh-mc-dropout")
    ...
    ckpt_path = BASE / f"outputs/abmil/{args.encoder}/fold{args.fold}/best.ckpt"
    if not ckpt_path.exists():
        raise FileNotFoundError(f"No trained checkpoint at {ckpt_path}; run train_abmil.py first")
    ...
    run_name = args.run_name or f"{args.encoder}_{args.dataset}_fold{args.fold}_mc{args.n_passes}"
    wandb.init(
        ...
        tags=[args.encoder, args.dataset, f"fold{args.fold}", "mc_dropout"],
    )

    encoder_root, encoder_subdir = ENCODER_DIRS[args.encoder]
    in_dim      = ENCODER_FEATURE_DIM[args.encoder]
    feature_dir = BASE / "data/processed/features" / args.dataset / encoder_root / encoder_subdir
    ...
    out_dir = BASE / f"outputs/mc_dropout/{args.dataset}/{args.encoder}"
    ...
    artifact = wandb.Artifact(
        f"mc-dropout-{args.encoder}-{args.dataset}-fold{args.fold}", type="mc_dropout_scores"
    )
