"""
Extract frozen ABMIL slide-level embeddings (z vectors) for the codeletion
(1p/19q, IDH-mutant subset) target -- codel counterpart of
extract_abmil_embeddings.py. Needed for laplace_infer_codel.py, which fits
its Laplace posterior directly on these cached z vectors + the classifier
head rather than re-running the full WSI forward pass.

Loads each fold's outputs/abmil_codel/<encoder>/fold<N>/best.ckpt, runs
inference on ALL codel-labelled slides in the given dataset, saves z
vectors + labels + predictions to
outputs/embeddings_codel/<dataset>/<encoder>/fold<N>.npz

Usage:
    python scripts/training/extract_codel_embeddings.py --encoder uni2 --dataset ebrains
    python scripts/training/extract_codel_embeddings.py --encoder uni2 --dataset tcga
    python scripts/training/extract_codel_embeddings.py --encoder uni2 --dataset ipd_brain
"""
...
def main():
    ...
    feature_dir = (BASE / args.feature_root / args.dataset
    ...
    out_dir = BASE / f"outputs/embeddings_codel/{args.dataset}/{args.encoder}"
