"""
Deep-ensemble uncertainty for codeletion (1p/19q, IDH-mutant subset) --
codel counterpart of deep_ensemble_infer.py. Loads all --init_seeds
checkpoints already trained by train_abmil_codel_deep_ensemble.py for one
encoder/fold and runs one deterministic (dropout off) forward pass per
member per slide.

    predictive_entropy (total, from the K-member mean prob)
        = aleatoric (mean of the K per-member entropies)
        + epistemic_mi (their difference, clipped at 0)

Usage:
    python scripts/training/deep_ensemble_infer_codel.py --encoder uni2 --fold 0 --dataset tcga
"""
...
def load_member(encoder: str, fold: int, init_seed: int, in_dim: int) -> ABMIL:
    ckpt_path = BASE / f"outputs/abmil_codel_deep_ensemble/{encoder}/fold{fold}/init{init_seed}/best.ckpt"
    if not ckpt_path.exists():
        raise FileNotFoundError(
            f"No deep-ensemble checkpoint at {ckpt_path}; "
            "run train_abmil_codel_deep_ensemble.py first"
        )
...
def main():
    ...
    parser.add_argument("--init_seeds", type=str, default="43,44,45,46",
                         help="Comma-separated init_seeds trained by "
                              "train_abmil_codel_deep_ensemble.py for this encoder/fold.")
    ...
    parser.add_argument("--wandb_project", type=str, default="idh-codel-deep-ensemble-infer")
    ...
    run_name = args.run_name or f"codel_{args.encoder}_{args.dataset}_fold{args.fold}_de{len(init_seeds)}"
    wandb.init(
        ...
        tags=[args.encoder, args.dataset, f"fold{args.fold}", "codel", "deep_ensemble"],
    )
    ...
    out_dir = BASE / f"outputs/deep_ensemble_codel/{args.dataset}/{args.encoder}"
    ...
    artifact = wandb.Artifact(
        f"deep-ensemble-codel-{args.encoder}-{args.dataset}-fold{args.fold}", type="deep_ensemble_scores"
    )
